...
process download_data {
    ...
    spe <- read10xVisium(here("SpatialBenchVisium_460_462_463_FFPE_manual/preprocessed/FFPE_1_WT_460/outs/"),
    ...
    print(dim(spe))
    colData(spe)[,"sample_id"] <- rep("FFPE_1_WT_460",dim(spe)[2])
    ...
    spe <- read10xVisium(here("SpatialBenchVisium_460_462_463_FFPE_manual/preprocessed/FFPE_1_WT_462/outs/"),
                     type = "sparse",   # use sparse (not HDF5) format
                     data = "raw",     
                     images = "lowres", # specify which image(s) to include
                     load = TRUE)      # specify whether or not to load image(s)
    ...
    print(dim(spe))
    colData(spe)[,"sample_id"] <- rep("FFPE_1_WT_462",dim(spe)[2])
    colData(spe)[,"preservation"] <- rep("FFPE",dim(spe)[2])
    colData(spe)[,"placement"] <- rep("manual",dim(spe)[2])
    colData(spe)[,"experiment"] <- rep("probe",dim(spe)[2])
    ...
    spe <- read10xVisium(here("SpatialBenchVisium_460_462_463_FFPE_manual/preprocessed/FFPE_1_WT_463/outs/"),
                     type = "sparse",   # use sparse (not HDF5) format
                     data = "raw",     
                     images = "lowres", # specify which image(s) to include
                     load = TRUE)      # specify whether or not to load image(s)
    ...
    print(dim(spe))
    colData(spe)[,"sample_id"] <- rep("FFPE_1_WT_463",dim(spe)[2])
    colData(spe)[,"preservation"] <- rep("FFPE",dim(spe)[2])
    colData(spe)[,"placement"] <- rep("manual",dim(spe)[2])
     colData(spe)[,"experiment"] <- rep("probe",dim(spe)[2])
    ...
    save(spe, file=here("SpatialBenchVisium_460_462_463_FFPE_manual","results", "SpatialBenchVisium_460_462_463_FFPE_manual_spe_qc.Rdata"))
    """
}
