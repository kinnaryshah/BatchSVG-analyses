...
process download_data {
    ...
    spe <- read10xVisium(here("SpatialBenchVisium/preprocessed/OCT_WT_709/outs/"),
                     type = "sparse",   # use sparse (not HDF5) format
                     data = "raw",     
                     images = "lowres", # specify which image(s) to include
                     load = TRUE)      # specify whether or not to load image(s)
    ...
    overlap_genes <- intersect(intersect(rownames(spe1),rownames(spe2)),intersect(rownames(spe3),rownames(spe4)))
